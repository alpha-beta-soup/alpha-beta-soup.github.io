...
publishedAt: 2016-04-28T21:02:00+1300
summary: "A narrative arc of the saga of the NZTA's Crash Analysis System (CAS) data, from its publication to its removal and the subsequent fallout."
category: [Professional,Personal]
tags: ["Open Data", "NZTA", "Crash Analysis System", "CAS", "Data Privacy", "Data Access", "Data Visualisation"]
---

For several years, up until at least June 2015, the New Zealand Transport Agency (NZTA) published the data underlying their Crash Analysis System (CAS). This included information on every Police-attended road crash or accident in New Zealand, with attendant information on the environment it occurred in, and what was deemed to have contributed to the accident. While the method of publishing this data was primitive,
<Marginalia side="right" marker="*">It consisted of one CSV file for each calendar year, with each record in the file heavily encoded. Decoding required automation and a lot of sifting through several PDFs, and a tool to get machine-readable tables from them. An example record is `'Ashburton District','MOORE ST','100','N',' ','WILLIAM ST','201531749','07/02/2015','Sat','2335','EA','CN1V','103A 129A ','M','R','D','DO','F ',' ',' ','C','050','0','0','0','','','1499510','5137544',` and yes that whitespace in `'F '` is actually meaningful.</Marginalia>
...
When I found this dataset one afternoon in 2014, I set to work making a prototype [map visualisation](https://spatialparalysis.xyz/national-crash-statistics/). My initial concern at the time was just trying to see the frequency of different type of accidents involving cyclists along my commute home between Wellington and Petone. I wanted to know this for my own curiousity and safety, but I also wanted to share it with members of the [Cycle Aware Wellington Facebook group](https://www.facebook.com/groups/166036183455868/), because I thought it would be useful to them.
...
![](/images/posts/cas/nzcm-v0-fb.png)

It was buggy as Hell but seemed well-received, and I was inspired to keep working on it.

![](/images/posts/cas/nzcm-v0-db-comments.png)

Here's how I first decoded the CAS records:

![](/images/posts/cas/nzcm-v0.jpg)

Here's how it's done [now](https://spatialparalysis.xyz/national-crash-statistics/):

![Astute readers will notice that this is the decoded visual summary of the raw data pasted in the margin further up the page. Would you ever have worked that out?](/images/posts/cas/asburton-example.png)
...
![](/images/posts/cas/actual-cas-wtf.png)

The [training](/public/videos/posts/cas/cas9-video.wmv) [videos](/public/videos/posts/cas/cas12-video.wmv) are a little bit sad/funny. They give an insight into the *awesome UI* I was missing out on by not signing a license agreement and using their system to view a text file.<Marginalia side="right" marker="*">Credit where it's due: there's a lot in that bloated interface that I wouldn't even attempt to implement. I'm sure it's useful for a number of use cases. It just would be great *if it were possible to use it*.</Marginalia>
...
### [`CAS-Privacy-Agreement-v1.docx`](/public/posts/cas/CAS-Privacy-Agreement-v1.pdf)

This is the privacy agreement that must be signed to use the NZTA's CAS viewer. In particular, it stipulates that no-one can access or use CAS data unless they have been entitled to do so by the NZTA in writing. I figured that providing the *data* for download absolved me from this, and these statements were only to be found in relation to accessing the Citrix system or the *viewer*. I was never told otherwise. There was no fee associated with using CAS.
...
To my informal and naïve tastes, there's a considerable amount of legalese in this document, so I'd really appreciate any interpretations of it (leave a comment).
...
![](/public/documents/posts/cas/heiroglyphics.png)
...
<Dialogue>
  <DialogueMessage name="Richard" side="left">
    I was looking to update my record of the CAS data, but the page I have used in the past no longer exists: [https://www.nzta.govt.nz/resources/crash-analysis-system-data/](https://www.nzta.govt.nz/resources/crash-analysis-system-data/)
  ...
  <DialogueMessage name="NZTA" side="right">
  ...
  <DialogueMessage name="Richard" side="left">
    ...
    However I'm using the CAS data to make a public web map ([https://spatialparalysis.xyz/national-crash-statistics/](https://spatialparalysis.xyz/national-crash-statistics/)) in collaboration with some others. **Will this review mean that my application will not be able to present the disaggregate information to the public?**
  </DialogueMessage>
  <DialogueMessage name="Richard (after a week without a response)" side="left">
...
<Dialogue>
    <DialogueMessage name="Richard" side="left">
        Thank you so much for the clarification about accessing the CAS data.
...
I have some plans my [crash map](https://spatialparalysis.xyz/national-crash-statistics/), but they're mostly shelved until I know that the raw CAS data will be available again:
